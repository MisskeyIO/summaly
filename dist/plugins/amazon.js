...
export async function summarize(url) {
    ...
    const description = $('#productDescription').text() || $('meta[name="description"]').attr('content');
    const thumbnail = $('#landingImage').attr('src');
    const playerUrl = $('meta[property="twitter:player"]').attr('content') || $('meta[name="twitter:player"]').attr('content');
    ...
    return {
        ...
        player: {
            url: playerUrl || null,
            width: playerWidth ? Number.parseInt(playerWidth) : null,
            height: playerHeight ? Number.parseInt(playerHeight) : null,
